"""planner — the prerequisite state machine (mirrors PDFDRILL's planner.py).
...
``done_when`` already holds is simply skipped.
  ...
  chatdrill steps <cmd> <id>      show the chain: what's done, what would run
  chatdrill <cmd> <id> --ensure   auto-run the missing prerequisites, then <cmd>

SAFETY: only offline, idempotent prerequisites are declared and auto-run.
"""
...
def detect(spec: str, sc: Sidecar) -> bool:
    """Is a `done_when` spec satisfied for this chat?
      fact:NAME    the sidecar carries that fact
      model        the chatmodel.json artifact exists
      artifact:REL a blob at REL exists"""
    if spec == "model":
        return sc.has_blob("chatmodel.json")
...
def resolve_steps(target: str, sc: Sidecar) -> tuple[list[str], set[str]]:
    """(ordered steps incl. target, satisfied set) for `target` on this chat."""
    man = load_manifest()
...
def describe(target: str, sc: Sidecar) -> str:
    ...
    chat = sc.chat_id[:12]
    if not prereqs:
        return (f"`{target}` for {chat}…: prerequisites satisfied "
                f"({', '.join(sorted(sat)) or 'none required'}) — runs directly.")
    return (f"`{target}` for {chat}… would run, in order:\n  "
            + " → ".join(steps)
